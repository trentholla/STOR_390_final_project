...
For our model fitting, we randomly selected 80% of the data to serve as our training set. 
```{r train}
...
The first models we tried were simple linear regressions of the final grade to the other variables in the data set. Before this occured, we split the data into training and test data so that we can test our predictive model and avoid overfitting. The first model included every variable in the data set. This was more of a control model, for it does not reveal anything substantial about the data other than that all factors affect a student's final grade. The second model included all variables that were deemed significant at a 90% confidence interval, in order to narrow down important variables. The third model removed more variables that were no longer significant in the second model. Since G1 and G2 are the most significant variables, they were further tested with squared and interaction terms. The fourth model added sqaured values of the two test grades, G1 and G2. Model five added an interaction variable between the two test grades.
```{r}
...
model4 <- lm(G3 ~ paid + goout + Dalc + absences + G1 + G2 +I(G1^2) + I(G2^2) + class, train)
MSE4 <- mean(model4$residuals^2)

model5 <- lm(G3 ~ paid + goout + Dalc + absences + G1 + G2 + I(G1*G2) + class, train)
